...
> 完整前后端 AI Agent 工程项目，覆盖多工具编排、RAG、SQL Guardrails、异步工作流、可追踪性与评测。
...
| Async Submit p95 Latency | 0.007 ms（local benchmark，仅测 `run_id` 返回） | `backend/app/eval_results/async_benchmark.json` |
...
本仓库不应被描述为“生产环境大规模验证”。它是一个面向生产思路的工程实现，如果真正上线，还需要补充租户隔离、SSO、监控、备份、限流、告警、事故处理和生产部署加固。
...
- Eval Pass Rate = passed cases / total cases，来自 `python -m app.scripts.run_eval --type all`。
- RAG Eval Pass Rate = passed RAG cases / total RAG cases，来自 `python -m app.scripts.run_eval --type rag`。
- Unsafe SQL Block Rate = blocked unsafe SQL cases / total unsafe SQL cases，来自 `python -m app.scripts.run_eval --type sql-guardrails`。
- Tool Calling Eval Pass Rate = passed tool cases / total tool cases，来自 `python -m app.scripts.run_eval --type tool`。
- Regression Pass Rate = passed regression cases / total regression cases，来自 `python -m app.scripts.run_regression`。
- Async Submit Latency = `python -m app.scripts.benchmark_async` 返回 `run_id` 的耗时，不代表完整 Agent 工作流完成耗时。
- Trace Replay Success Rate = 可通过 `run_id` 查询到 run、steps、tool call、SQL log、audit log 和 error 的失败 Agent Run / 被检查失败 Agent Run 总数，来自 `python -m app.scripts.benchmark_trace`。
- 公开验证使用 Mock providers 和 synthetic demo data，不需要真实模型 API Key，也不包含真实企业数据。
...
本仓库主要是完整前后端 AI Agent 工程项目。Docker-first 本地 Demo 已使用 Mock providers 和 synthetic demo data 完成验证。
...
在已验证的 Stage-12 项目快照中，所有计划开发阶段均已完成；当前仓库是一个完整前后端 AI Agent 工程项目，包含阶段验收材料、最终验收指南、公开安全检查、CI/CD workflows、支持 Mock providers 的本地 Demo、公开安全的 synthetic demo data 与文档，并包含本地 Demo 截图和 GIF 录屏，可用于技术展示、Demo 演示和面试讲解。
